...
zokou({nomCom : "igdl" , categorie : "Download"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader 𝑷𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 *𝑳𝒂𝒎𝒃𝒐𝒓𝒈𝒉𝒊𝒏𝒊*",gifPlayback : false },{quoted : ms}) 
    }
    else {
        zk.sendMessage(dest,{image : {url : igvid.data.data.data[0].url},caption : "ig image downloader 𝑷𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 *𝑳𝒂𝒎𝒃𝒐𝒓𝒈𝒉𝒊𝒏𝒊*"})
    }
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       ...
       zk.sendMessage(dest, { video: { url: result.hd  }, caption: 'facebook video downloader 𝒑𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 *𝑳𝒂𝒎𝒃𝒐𝒓𝒈𝒉𝒊𝒏𝒊*' }, { quoted: ms });
...
async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       zk.sendMessage(dest,{image : { url : result.thumbnail}, caption : caption},{quoted : ms}) ;
       zk.sendMessage(dest, { video: { url: result.sd  }, caption: 'facebook video downloader 𝑷𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 *𝑳𝒂𝒎𝒃𝒐𝒓𝒈𝒉𝒊𝒏𝒊*' }, { quoted: ms });
